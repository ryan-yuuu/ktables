...
class TestGroupedConstruction:
    ...
    def test_enable_idempotence_forwarded_to_inner_writer(self) -> None:
        default = GroupedKafkaTableWriter(bootstrap_servers=BOOTSTRAP, topic="unit.grouped.idem", value_encoder=bytes)
        custom = GroupedKafkaTableWriter(
            bootstrap_servers=BOOTSTRAP, topic="unit.grouped.idem", value_encoder=bytes, enable_idempotence=True
        )
